...
ordering persists on a trained $n{=}100$ checkpoint. On VMAS Dispersion ($n{=}10$,
...
variance at reduced per-call cost, plus a deterministic expander
ablation showing near-unit relative error
($\SNDG^{\mathrm{u}}/\SND\in[0.9987,1.0013]$) at
...
expander ablation), while training is demonstrated to $n{=}100$ for
online PPO and to $n{=}50$ for DiCo set-point control
...
The same Bernoulli-$0.1$ substitution extends to $n{=}50$ and three
distinct set points $\SND_{\mathrm{des}}\in\{0.12,0.14,0.15\}$ (three
seeds each), tracking each target to within $0.5\%$ relative error
while preserving the monotonic reward--diversity ordering; see
Appendix~\ref{app:dico-n50-sweep}.
...
\citep{bettini2025snd}; the main DiCo integration uses Dispersion at
a single $\SND_{\mathrm{des}}{=}0.1$ over three seeds, with a
supplementary three-seed, three-set-point sweep at $n{=}50$ in
Appendix~\ref{app:dico-n50-sweep}.
...
% ------------------------------------------------------------------
% Appendix: DiCo + Bernoulli-0.1 set-point sweep at n=50 (3 seeds x 3
% desired_snd values).  Results files live at
%   ControllingBehavioralDiversity-fork/results/neurips_final_n50_setpoint_sweep/
% and the summary figure/table at
%   results/dico_n50_sweep/{n50_setpoint_sweep.pdf, n50_setpoint_sweep_summary.csv}.
\section{DiCo + Bernoulli-$0.1$ set-point sweep at $n{=}50$}
\label{app:dico-n50-sweep}

We extend the closed-loop experiment of
Section~\ref{sec:experiments:dico-dispersion} along two axes: team
size (from $n{=}10$ to $n{=}50$, ${\approx}5\times$ more agents) and
the diversity set point itself (from the single
$\SND_{\mathrm{des}}{=}0.1$ used in the main text to three values
$\SND_{\mathrm{des}}\in\{0.12,0.14,0.15\}$). Each
(seed, set point) cell is one independent $167$-iteration IPPO run
(${\approx}10^7$ frames) with Bernoulli-$0.1$ Graph-SND as the sole
diversity estimator fed to DiCo; hyperparameters match
...
per-agent tensors). The expected edge count fed to the controller is
$\mathbb{E}[|E(G_{0.1})|]{=}0.1\binom{50}{2}{=}122.5$ out of
$\binom{50}{2}{=}1{,}225$ pairs ($10\%$ edge density).

Figure~\ref{fig:dico-n50-sweep}(a) shows the per-iteration applied
$\SND$ (post-scaling, the quantity DiCo regulates) for every
(seed, set point) cell: after a ${\sim}25$-iteration transient each
run settles onto its horizontal target line and stays there for the
remaining ${\sim}140$ iterations of training. Panel~(b) shows episode
reward on the same runs: higher set points yield monotonically higher
terminal reward, consistent with Dispersion rewarding role
specialisation among a large team. Table~\ref{tab:dico-n50-sweep}
aggregates the late-window ($50$-iteration tail) statistics across
seeds. The controller tracks each set point to within
$0.42$--$0.53\%$ relative error (i.e.\ within $8\times 10^{-4}$ in
absolute $\SND$), with cross-seed standard error on the applied
$\SND$ of at most $1.2\times 10^{-4}$. Median per-call metric time is
${\approx}2.05\,\text{ms}$ across all cells, regardless of set
point---consistent with the $\binom{n}{2}/|E|$ cost invariant of
Proposition~\ref{prop:complexity}. The result extends the
closed-loop conclusion of Section~\ref{sec:experiments:dico-dispersion}
along both scale and set-point axes: at $n{=}50$, with a
random-graph estimator that queries only $10\%$ of pairs per
iteration, DiCo regulates diversity to multiple distinct targets with
sub-percent tracking error on a real multi-agent RL task.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/dico_n50_setpoint_sweep.pdf}
\caption{DiCo $+$ Bernoulli-$0.1$ Graph-SND at $n{=}50$ across three
set points (three seeds each). \textbf{(a)}~Applied
$\SND{=}\SND_t\,s_t$ per iteration; dotted lines mark
$\SND_{\mathrm{des}}\in\{0.12,0.14,0.15\}$. Curves are mean across
seeds; bands are $\pm 1$ std. All three runs converge to their
target within ${\sim}25$ iterations and stay on target for the
remaining ${\sim}140$ iterations of training. \textbf{(b)}~Mean
episode reward on the same runs, same colour coding. Higher set
points yield monotonically higher terminal reward.}
\label{fig:dico-n50-sweep}
\end{figure}

\begin{table}[ht]
\centering
\caption{N=50 DiCo $+$ Bernoulli-$0.1$ Graph-SND set-point sweep.
Aggregated across three seeds on the late window (last $50$
iterations). ``Applied $\SND$'' and ``reward'' are reported as
mean $\pm$ cross-seed standard error (SEM); ``rel.\ track.\ err.''
is the mean absolute tracking error divided by
$\SND_{\mathrm{des}}$; ``metric time'' is the across-cell mean of
per-run median per-call times. All runs use the same
Bernoulli-$0.1$ estimator with expected edge count $122.5$ out of
$1{,}225$ pairs.}
\label{tab:dico-n50-sweep}
\small
\begin{tabular}{cccccc}
\toprule
$\SND_{\mathrm{des}}$ & Applied $\SND$ & Rel.\ track.\ err.\ (\%) & Reward & Scaling $s_t$ & Metric time (ms) \\
\midrule
$0.12$ & $0.1203 \pm 1.2\!\times\!10^{-4}$ & $0.53$ & $0.295 \pm 0.016$ & $0.0159$ & $2.13$ \\
$0.14$ & $0.1404 \pm 0.7\!\times\!10^{-4}$ & $0.48$ & $0.338 \pm 0.008$ & $0.0184$ & $2.00$ \\
$0.15$ & $0.1503 \pm 0.1\!\times\!10^{-4}$ & $0.42$ & $0.350 \pm 0.005$ & $0.0183$ & $2.05$ \\
\bottomrule
\end{tabular}
\end{table}
