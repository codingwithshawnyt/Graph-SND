...
The experiments now support a simple decision rule for choosing $G$.
Use Bernoulli graphs when the goal is unbiased approximation of full
$\SND$ at the lowest runtime cost (sampling-estimator regime); use
structured fixed graphs (e.g., $k$-NN or communication topologies) when
the goal is local-semantic diversity over task-relevant interactions
(modeling regime); use random $d$-regular expanders when a
\emph{deterministic} sparse approximation is required, since they
deliver near-unit empirical ratios at $|E|{=}\Theta(n\log n)$ while
maintaining low forwarding index and strong distortion behavior
relative to matched-edge baselines.
