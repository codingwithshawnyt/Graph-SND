...
\begin{abstract}
Graph-SND provides a deterministic expander-sparsification of pairwise
diversity aggregation: random $d$-regular expanders achieve
$\SNDG^{\mathrm{u}}/\SND\in[0.9987,1.0013]$ at
$|E|{=}\Theta(n\log n)$ across $n\in\{50,100,200,500\}$, with
$2$--$39\times$ lower forwarding index than matched-edge sparse
baselines and persistence on a trained $n{=}100$ checkpoint. System
Neural Diversity (SND) quantifies behavioral heterogeneity in
...
$n$ and three hardware/call regimes. In a deterministic fixed-$G$
expander ablation, random $d$-regular graphs achieve
$\SNDG^{\mathrm{u}}/\SND\in[0.9987,1.0013]$ at
$|E|{=}\Theta(n\log n)$ across $n\in\{50,100,200,500\}$, and this
ordering persists on a trained $n{=}100$ checkpoint. On PettingZoo MPE
simple-spread at $n{=}10$ (discrete actions, categorical policies, TVD
distance), a three-seed panel shows that sparse Graph-SND estimators
track full SND with max absolute bias $2.92{\times}10^{-3}$ while
reducing wall-clock metric cost by $1.97$--$4.87\times$ depending on edge
budget. On VMAS Dispersion at $n{=}10$, substituting a $k{=}3$ $k$-NN graph,
Bernoulli-$0.1$ random subgraph, or random $d$-regular expander
Graph-SND ($d{=}\lceil\log_2 n\rceil$) for full SND inside the DiCo
diversity controller matches the $\SND_{\mathrm{des}}{=}0.1$ set
point to within ${\approx}2\%$ relative error (typically within
$10^{-3}$ for $k$-NN and Bernoulli-$0.1$) and matches full-$\SND$
reward within seed variance; a paired three-seed, three-set-point head-to-head at
$n{=}50$ trains both Bernoulli-$0.1$ Graph-SND and full SND as the
DiCo estimator and finds statistically indistinguishable reward and
tracking (sub-$1\%$ relative error for both) at a measured
${\sim}9.5{\times}$ cheaper per-call metric cost for
Bernoulli-$0.1$.
\end{abstract}
% ============================================================
