...
sampled edges $m$, independent of $n$. We prove six properties
(recovery, non-negativity, graph-automorphism invariance, complexity,
unbiasedness, concentration) and verify them on VMAS Multi-Agent Goal
...
$\SND$ (random $G$). (ii) Six formal results
(Section~\ref{sec:theory}): exact recovery on $K_n$, non-negativity,
graph-automorphism invariance, $O(|E|)$ complexity, estimator
unbiasedness, and Hoeffding-type concentration; proofs in
Appendix~\ref{app:proofs}. (iii) Experiments on VMAS
...
Tracking fidelity at scale is inherited from
Theorem~\ref{thm:concentration}: Bernoulli sampling at $n{=}500$,
$p{=}0.1$ draws $m\!\approx\!12{,}475$ edges ($2.5\times$ more than
at $n{=}100$, $p{=}0.1$), tightening the concentration radius of
Eq.~\eqref{eq:concentration} by a factor
$\sqrt{12{,}475/495}\!\approx\!5.0$ relative to
Section~\ref{sec:experiments:scaling}, where the two curves already
overlap at every logged iteration. Full CPU timing curves and
absolute wall-clock values at all $(n, p)$ cells appear in
...
unbiasedness/concentration). The most immediate downstream
application is DiCo \citep{bettini2024dico}: $\SNDG$ drops in as the
control target, with $G$ selecting between local diversity control
and a compute-subsampled variant, as validated on VMAS Dispersion.
...
$G$ we exercise is the $k$-NN graph inside DiCo, and no deterministic
distortion bound $|\SNDG-\SND|\le f(G,\mathbf{D})$ is
proved---only the random-$G$ guarantee
...
(iii) a \emph{deterministic fixed-$G$ distortion bound} under
regularity conditions on $\mathbf{D}$ along $G$; and (iv) a
broader DiCo$+\SNDG$ study, over larger $n$, tasks, set-points,
and the interaction of sampling noise in $\widehat{\SND}(G_p)$ with
DiCo's set-point regulation.
\section{Conclusion}
\label{sec:conclusion}

We introduced Graph-SND, a graph-structured generalisation of SND
that replaces the uniform average over $\binom{n}{2}$ pairs with a
normalised weighted mean over the edges of a user-specified graph
$G$: a localised diversity measure when $G$ is fixed, and an unbiased
Horvitz--Thompson sampling estimator of $\SND$ when edges are drawn
independently. Six formal results (Section~\ref{sec:theory}) and
experiments across $n\in\{4,8,16,100,250,500\}$ on VMAS Goal
Navigation and a DiCo closed loop on Dispersion confirm that $\SNDG$
is a drop-in replacement for $\SND$: recovery is numerically exact,
concentration is respected on every cell, wall-clock cost tracks
$\binom{n}{2}/|E|$ across a $125\times$ span of $n$, and both $k$-NN
and Bernoulli-$0.1$ substitutions inside DiCo match the full-$\SND$
set point to within $10^{-3}$ and full-$\SND$ reward within seed
variance. The choice of $G$ selects whether the signal measures
local diversity or approximates global diversity at reduced cost.
\codelocation
