...
\citep{heydemann1989forwarding,leighton1999multicommodity}, and a
spectral nuclear-norm refinement. (iii) Experiments on VMAS
\citep{bettini2022vmas} and PettingZoo verify recovery, concentration,
and wall-clock scaling through $n=500$; a 500-iteration $n=100$ PPO run
...
Graph-SND preserves reward and set-point tracking within observed seed
variation while reducing per-call metric cost by ${\sim}9.5\times$.
...
Core checks on VMAS Goal Navigation verify exact recovery on $K_4$,
Horvitz--Thompson unbiasedness at $n=8$, and zero concentration-bound
violations across $24{,}000$ finite-population draws at $n=16$
...
Goal Navigation for $500$ PPO iterations. Every $25$ iterations we log
...
(Appendix~\ref{app:verify}, Figure~\ref{fig:timing-n500}).
...
\citep{bettini2024dico} on VMAS Dispersion. A $n=10$ study shows that
...
(Appendix~\ref{app:dico-n10}). The stronger closed-loop evidence is the
$n=50$ head-to-head in Table~\ref{tab:dico-n50-main}: each of three
seeds and three set points is trained twice, once with Bernoulli-$0.1$
Graph-SND and once with full SND. Bernoulli-$0.1$ tracks all targets to
sub-$1\%$ relative error, yields paired reward differences centered near
zero, and reduces metric cost by $9.24$--$9.63\times$, matching the
$10\times$ pair-count prediction.
...
but a fully unconditional distribution-dependent theory remains open.
...
\paragraph{GPU frozen-init timing through $n=500$.}
Figure~\ref{fig:timing-n500} reports the large-$n$ timing sweep
summarized in Section~\ref{sec:experiments:scaling}: Bernoulli-$p$
Graph-SND follows the structural $1/p$ speedup, and Bernoulli-$0.1$
remains near $10\times$ through $n=500$.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/timing_n500.pdf}
\caption{Wall-clock scaling of $\SNDG^{\mathrm{u}}(G_p)$ versus $\SND$
across $n\in\{4,\ldots,500\}$ and three hardware/call regimes. The
large-$n$ GPU timing sweep follows the structural $1/p$ prediction; at
$p=0.1$, speedups remain near $10\times$ through $n=500$.}
...
\section{DiCo at $n{=}10$: sparse graph choices}
\label{app:dico-n10}

Figure~\ref{fig:dico-dispersion-knn} shows the VMAS Dispersion
$n=10$ study summarized in Section~\ref{sec:experiments:dico-dispersion}.
DiCo with full SND, $k$-NN Graph-SND, Bernoulli-$0.1$ Graph-SND, and
expander Graph-SND all track $\SND_{\mathrm{des}}=0.1$ within $2\%$
relative error and match reward within seed variation, while the sparse
variants reduce per-call metric cost in the expected pair-count order.
