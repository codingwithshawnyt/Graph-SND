...
ordering persists on a trained $n{=}100$ checkpoint. On VMAS Dispersion at $n{=}10$, substituting a $k{=}3$ $k$-NN graph,
...
trained $n{=}100$ checkpoint panel.
...
\paragraph{Discrete-action instantiation (categorical + TVD).}
To surface the distance-agnostic claim in the main experimental
story, we also run a synthetic discrete-action sanity check with
categorical policies and total-variation distance (TVD) as $d(i,j)$
(full protocol and table in Appendix~\ref{app:verify},
Table~\ref{tab:discrete-tvd-sanity}). Across $2{,}000$ Bernoulli draws
per $p\in\{0.1,0.3,0.5\}$, empirical bias remains near zero
($|\mathrm{bias}|\le 7.48\times 10^{-4}$) and the Hoeffding violation
rate at $\delta{=}0.1$ is zero in all cells. This is not a new RL
benchmark claim; it is a direct sanity check that the same
unbiasedness/concentration guarantees transfer to a non-Gaussian
distance without changing the graph-aggregation layer.
...
Table~\ref{tab:discrete-tvd-sanity}). The two interpretations of
...
$n{=}500$) plus online training at $n{=}100$ and DiCo control on
Dispersion (main: $n{=}10$, supplementary: $n{=}50$) confirm that
...
\paragraph{Distance-agnostic discrete sanity check (categorical + TVD).}
To verify the distance-agnostic claim beyond the Gaussian-$\Wass$
instantiation, we run a small synthetic discrete-action check with
$n{=}10$ agents and $k{=}5$ actions. Each agent policy is sampled from a
Dirichlet$(\alpha{=}1)$ categorical distribution, pairwise dissimilarity
is total variation distance (TVD), and $\widehat{\SND}(G_p)$ is computed
over $2{,}000$ Bernoulli-$p$ graph draws for
$p\in\{0.1,0.3,0.5\}$ (Table~\ref{tab:discrete-tvd-sanity}). The
empirical bias is near zero in all cells and the Hoeffding violation
rate at $\delta{=}0.1$ is zero in all cells, matching the bounded-$d$
theory of Proposition~\ref{prop:unbiased} and
Theorem~\ref{thm:concentration}. This check is generated by
\texttt{experiments/discrete\_tvd\_sanity.py} and is intended as a
sanity validation of the estimator guarantees under a non-Gaussian
dissimilarity, not as a new RL benchmark claim.
...
\toprule
$p$ & mean $|E|$ & $\SND$ & $\E[\widehat{\SND}]$ & bias & std & viol. rate \\
\midrule
0.1 & 4.59 & 0.44990 & 0.44976 & $-1.43{\times}10^{-4}$ & 0.07737 & 0.000 \\
0.3 & 13.47 & 0.42005 & 0.42080 & $+7.48{\times}10^{-4}$ & 0.04226 & 0.000 \\
0.5 & 22.32 & 0.48500 & 0.48440 & $-6.02{\times}10^{-4}$ & 0.02451 & 0.000 \\
\bottomrule
\end{tabular}
\caption{Discrete-action sanity check with categorical policies and TVD
($n{=}10$, $k{=}5$, $2{,}000$ graph draws per $p$). `viol. rate' is the
fraction of draws violating the Hoeffding radius at $\delta{=}0.1$;
all cells are zero.}
\label{tab:discrete-tvd-sanity}
\end{table}
