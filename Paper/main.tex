...
A deterministic companion to Theorem~\ref{thm:concentration} holds for
fixed $G$: Theorem~\ref{thm:distortion}
(Appendix~\ref{app:proofs}) sandwiches
$\SND/\SNDG^{\mathrm{u}}$ between $|E|/|\Pairs|$ and
$|E|\pi(G)/|\Pairs|$, where $\pi(G)$ is the edge forwarding
index~\citep{heydemann1989forwarding}; for $d$-regular spectral
expanders the upper ratio is $\mathcal{O}(\log n)$ at $\Theta(n\log n)$
...
Theorem~\ref{thm:distortion} bounds the worst-case distortion for
...
Theorem~\ref{thm:distortion} (e.g.\ variance-aware Bernstein-type
bounds on $\{d(i,j)\}_{\{i,j\}\in E}$) and an empirical study of
expander-graph fixed-$G$ designs; and (iv) a broader
DiCo$+\SNDG$ study, over larger $n$, tasks, set-points,
...
If $G$ is a $d$-regular Cayley graph on $n$ vertices with bounded
spectral gap, then Theorem~\ref{thm:distortion} and
$\pi(G)=\mathcal{O}(n\log n/d)$ give
